...
[role="lead"]
La taille d'un partage SMB 3.0 peut chuter à zéro dans System Center Virtual Machine Manager (SCVMM) lorsque la réinitialisation des quotas prend beaucoup de temps dans ONTAP en raison d'une activité d'E/S importante, ce qui empêche le provisionnement de nouvelles machines virtuelles. Résolvez ce problème en ajustant les paramètres de quota.
