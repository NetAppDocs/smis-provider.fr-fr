...
[role="lead"]
NetApp SMI-S Provider peut ne pas parvenir à se connecter à `localhost:5989` si les connexions HTTPS sont désactivées, si le port HTTPS est mal configuré ou si le fournisseur ne répond plus. Vérifiez vos paramètres de connexion HTTPS et redémarrez le fournisseur pour résoudre le problème.
