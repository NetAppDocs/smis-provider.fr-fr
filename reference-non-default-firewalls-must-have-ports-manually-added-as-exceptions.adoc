...
[role="lead"]
Si vous utilisez un pare-feu autre que le pare-feu Windows par défaut, NetApp SMI-S Provider pourrait ne pas être en mesure de communiquer avec les clients ou d'envoyer des indications, sauf si les ports requis sont ajoutés comme exceptions. Ajoutez ces ports à la configuration de votre pare-feu pour résoudre le problème.
