...
[role="lead"]
Un  `Network path not found` message lors du déploiement d'une machine virtuelle sur un partage SMB indique généralement un problème DNS dans lequel l'hôte ne possède pas encore d'enregistrement sur le serveur DNS. Résolvez ce problème en mettant à jour l'enregistrement DNS ou le fichier host.
